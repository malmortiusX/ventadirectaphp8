...
@section('content')
    <div class="row my-2">
        <div class="col s12 mb-2 center">
            ...
            <h3 class="subtitle subtitle-2 mb-0 mt-1" style="font-family: 'Gilroy';">IMPRESIÓN DE GUÍAS DE TRANSPORTE</h3>
        </div>
    </div>
    <form action="{{ route('guias.index.fenix') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="row mb-0 mt-1">

            <div class="col s12 mb-2 center col-dropify">
                <label class="dropify-label">Archivo</label>
                <input type="file" class="dropify" id="archivo" name="archivo" accept=".P1*, .P2*, .P3*, .P4*, .P5*, .P6*, .P7*, .P8*, .P9*, P0*" data-default-file="{{ isset($documento) && $documento->url && Storage::disk('private')->exists($documento->url) ? Storage::disk('private')->url($documento->url) : '' }}"/>
            </div>
            {{-- <div class="col s12">
        ...
        <div class="row mb-0 mt-1">
            ...
            <div class="col s12 center-align mt-2">
                <button class="mdc-button mdc-button--raised mdc-button--large" id="submit">
                    <span class="mdc-button__label fz-15 mr-1">Imprimir</span>
                    <i class="fal fa-truck-moving fz-15 send"></i>
                    <i class="fas fa-spinner fz-15 sending" style="display: none"></i>
                </button>
            </div>
