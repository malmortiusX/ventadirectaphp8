...
@section('content')
    <form action="{{ route('pedidos.semanales.close', ['ano' => $pedido->ano, 'semana' => $pedido->semana]) }}" method="POST" id="main-form">
        @csrf
        <div class="container-full">
            <div class="row">
                <div class="col s12 m6">
                    <h1 class="title">P. Semanal Pollo N° {{ $pedido->id_pedido }}</h1>
                    <div class="breadcrumbs full-width">
                        <a href="{{ route('dashboard') }}" class="breadcrumb">Dashboard</a>
                        <a href="{{ route('pedidos.semanales.index') }}" class="breadcrumb">Pedidos Semanales</a>
                        <a class="breadcrumb">Semana {{ $pedido->semana }} / {{ $pedido->ano }}</a>
                    </div>
                </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // Rutas de ajax
            var urlBuscar = '{!! route('ajax.pedidos.semanales.productos', ['busqueda' => 'null']) !!}';
            var urlGuardarProducto = '{!! route('ajax.pedidos.semanales.gurdarProducto') !!}';
            var urlEliminarProducto = '{!! route('ajax.pedidos.semanales.eliminarProducto') !!}';
            ...
            $('#agregar_producto_lista').click(function(){
                ...
                if (hoy < limite) {
                    ...
                    Swal.fire({
                        title: 'Denegado!',
                        text: 'La fecha limite para modificar el pedido de la semana '+ pedido.semana +' era el '+ limite.toLocaleDateString("es-ES", { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })+ ' a medio día',
                        icon: 'error'
                    });
                }
            });
