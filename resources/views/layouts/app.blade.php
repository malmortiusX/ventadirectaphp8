...
<head>
    ...
    <link rel="icon" href="{{ asset('imagenes/favicon/' . mb_strtolower($empresa->nombre) . '.svg') }}"
        sizes="32x32" />
    <link rel="icon" href="{{ asset('imagenes/favicon/' . mb_strtolower($empresa->nombre) . '.svg') }}"
        sizes="192x192" />
...
<body>
    <aside class="mdc-drawer mdc-drawer--dense mdc-drawer--dismissible mdc-drawer--modal" id="menu">
        <div class="mdc-drawer__header">
            ...
            <p class="subtitle subtitle-3 capitalize center my-0 nowrap">
                {{ mb_strtolower($usuario->nombres . ' ' . $usuario->apellidos) }}</p>
            @switch($usuario->nivel)
                ...
                @case('u_callcenter')
                    <label class="margin-auto">Call
                        Center{{ $usuario->codigo_vendedor ? ' - ' . $usuario->codigo_vendedor : '' }}</label>
                @break

                @case('u_supervisor')
                    <label
                        class="margin-auto">Supervisor{{ $usuario->codigo_vendedor ? ' - ' . $usuario->codigo_vendedor : '' }}</label>
                @break
        ...
        <div class="mdc-drawer__content">
            <div class="mdc-list">
                <a id="dashboard" class="mdc-list-item" href="{{ route('dashboard') }}">
                    ...
                    <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                            src="{{ asset('vendor/icons/store.png') }}"></i>
                ...
                @if ($usuario->nivel != 'u_supervisor')
                    <a id="pedidos-create" class="mdc-list-item" href="{{ route('pedidos.create') }}">
                        <span class="mdc-list-item__ripple"></span>
                        <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                src="{{ asset('vendor/icons/add-to-shopping-cart-e-commerce-button.png') }}"></i>
                    ...
                    @if ($usuario->nivel == 'u_ventadirecta')
                        <a id="abonos-index" class="mdc-list-item" href="{{ route('abonos.index') }}">
                            <span class="mdc-list-item__ripple"></span>
                            <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                    src="{{ asset('vendor/icons/commercial-ticket-of-shopping.png') }}"></i>
                    ...
                    <a id="pedidos-index" class="mdc-list-item" href="{{ route('pedidos.index') }}">
                        <span class="mdc-list-item__ripple"></span>
                        <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                src="{{ asset('vendor/icons/commercial-event-on-calendar.png') }}"></i>
                    ...
                    @if ($usuario->nivel == 'u_comercial')
                        <a id="pedidos-misPedidos" class="mdc-list-item" href="{{ route('pedidos.misPedidos') }}">
                            <span class="mdc-list-item__ripple"></span>
                            <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                    src="{{ asset('vendor/icons/shopping-bag-with-dollar-sign.png') }}"></i>
                            <span class="mdc-list-item__text">Mis Pedidos</span>
                        </a>
                        <a id="registro-facturas-anuladas" class="mdc-list-item" href="{{ route('ipbd') }}">
                            <span class="mdc-list-item__ripple"></span>
                            <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                    src="{{ asset('vendor/icons/commercial-ticket-of-shopping.png') }}"></i>
                            <span class="mdc-list-item__text">Registro Facturas Anuladas</span>
                        </a>
                    @endif
                    @if ($usuario->nivel != 'u_ventadirecta')
                ...
                @if ($usuario->nivel == 'u_movil' || $usuario->nivel == 'u_movil_especial' || $usuario->nivel == 'u_callcenter')
                    <a id="rutero" class="mdc-list-item" href="{{ route('rutero.index') }}">
                        <span class="mdc-list-item__ripple"></span>
                        <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                src="{{ asset('vendor/icons/timer-or-chronometer.png') }}"></i>
                ...
                @if ($usuario->nivel == 'u_movil' || $usuario->nivel == 'u_movil_especial' || $usuario->nivel == 'u_supervisor')
                    <a id="encuestas" class="mdc-list-item" href="{{ route('encuesta.show') }}">
                        <span class="mdc-list-item__ripple"></span>
                        <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                src="{{ asset('vendor/icons/e-commerce-search.png') }}"></i>
                        <span class="mdc-list-item__text">Encuesta</span>
                @endif
                @if ($empresa->nombre != 'Italcol')
                    @if (
                        $usuario->nivel == 'u_movil' ||
                            $usuario->nivel == 'u_movil_especial' ||
                            $usuario->nivel == 'u_comercial' ||
                            $usuario->nivel == 'u_administrador')
                        <a id="pedidos-semanales" class="mdc-list-item"
                            href="{{ route('pedidos.semanales.index') }}">
                            <span class="mdc-list-item__ripple"></span>
                            <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                    src="{{ asset('vendor/icons/shopping-basket-commercial-tool.png') }}"></i>
                        ...
                        @if ($empresa->pqrs)
                            <a id="pqrs" class="mdc-list-item" href="{{ route('pqrs.index') }}">
                                <span class="mdc-list-item__ripple"></span>
                                <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                        src="{{ asset('vendor/icons/verified-commercial-list.png') }}"></i>
                        ...
                        @if ($usuario->crea_clientes == 1)
                            <a id="crear-clientes" class="mdc-list-item" href="{{ route('clientes.create') }}">
                                <span class="mdc-list-item__ripple"></span>
                                <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                        src="{{ asset('vendor/icons/call-center-woman.png') }}"></i>
                                <span class="mdc-list-item__text">Crear Cliente</span>
                            </a>
                        @endif
                        @if ($usuario->bloquear_descuento == 0)
                            <a id="descuentos" class="mdc-list-item"
                                href="{{ route('descuentos.index', ['fecha_descuento' => date('Y-m-d')]) }}">
                                <span class="mdc-list-item__ripple"></span>
                                <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                        src="{{ asset('vendor/icons/commercial-percentage-discount-label.png') }}"></i>
                ...
                @if ($usuario->nivel == 'u_administrador')
                    <a id="registro-facturas-anuladas" class="mdc-list-item" href="{{ route('registro-facturas-anuladas.index') }}">
                        <span class="mdc-list-item__ripple"></span>
                    ...
                    <a id="actualizar" class="mdc-list-item" href="{{ route('actualizar') }}">
                        <span class="mdc-list-item__ripple"></span>
                        <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                src="{{ asset('vendor/icons/circular-arrows-couple.png') }}"></i>
                        <span class="mdc-list-item__text">Actualizar</span>
                    </a>
                    <a id="ipbd" class="mdc-list-item" href="{{ route('ipbd') }}">
                        <span class="mdc-list-item__ripple"></span>
                        <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                                src="{{ asset('vendor/icons/cube-outline.png') }}"></i>
                ...
                <a id="logout" class="mdc-list-item" href="{{ url('/logout') }}">
                    <span class="mdc-list-item__ripple"></span>
                    <i class="mdc-list-item__graphic mr-2" aria-hidden="true"><img class="icono"
                            src="{{ asset('vendor/icons/unlocked-circular-padlock.png') }}"></i>
    ...
    <main class="main-content container-full" id="main-content">
        <section class="div-main">
            @yield('content')
            <button class="btn btn-floating btn-large waves-effect waves-light" type="button" id="botonMenu"><i
                    class="fas fa-bars"></i></button>
        </section>
        <footer>
            <div class="row mb-0">
                <div class="col s12 m6">
                    <Label>Copyright © {{ date('Y') }} | <a class="link"
                            style="color: var(--mdc-theme-secondary);" href="https://avicolaelmadrono.com/"
                            target="_blank">Avicola El Madroño S.A.</a></Label>
                </div>
